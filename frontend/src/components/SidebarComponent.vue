<template>
  <aside
    class="top-0 left-0 z-40 w-64 h-screen transition-transform -translate-x-full sm:translate-x-0 bg-gray-800 text-white flex flex-col justify-between"
    aria-label="Sidebar">
    <div class="h-full px-4 py-6">
      <!-- Header -->
      <div class="flex items-center justify-between mb-6">
        <span class="text-2xl font-semibold text-blue-400">Note App</span>
        <button
          @click="addNewNote"
      ...
      <ul class="space-y-4">
        ...
        <li>
          <a
            href="#"
            @click.prevent="filterNotes('all')"
            class="flex items-center p-2 text-gray-300 rounded-lg hover:bg-gray-700 transition duration-300">
        ...
        <li>
          <button
            ...
            class="flex items-center w-full p-2 text-gray-300 rounded-lg hover:bg-gray-700 transition duration-300">
            ...
            <svg class="w-3 h-3 ml-2 transform transition-transform" :class="{ 'rotate-180': isDropdownOpen }"
          ...
          <ul
            :id="'dropdown-example'"
            :class="{ 'hidden': !isDropdownOpen, 'transition-all duration-300': true }"
            class="py-2 space-y-2 pl-8">
            <li
              ...
              <a
                href="#"
                @click.prevent="filterNotes(category.id)"
                class="flex items-center p-2 text-gray-300 rounded-lg hover:bg-gray-700 transition duration-300">
            ...
            <li v-if="showCategoryInput">
              <input
                ...
                class="w-full p-2 mt-2 text-white bg-transparent border-2 border-gray-600 focus:outline-none focus:border-blue-400" />
            </li>
            <li v-else>
              <button
        ...
        <li>
          <a
            href="#"
            @click.prevent="filterNotes('archived')"
            class="flex items-center p-2 text-gray-300 rounded-lg hover:bg-gray-700 transition duration-300">
    ...
    <button
      @click="logout"
      class="w-full p-2 bg-red-600 hover:bg-red-700 text-white rounded-lg transition duration-300 flex items-center justify-center">
